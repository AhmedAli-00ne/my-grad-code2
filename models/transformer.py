...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
        ...
        self.dense_layer = nn.Linear(self.d_model, self.d_model)
...
#         self.decoder = TransformerEncoder(encoder_layer, num_decoder_layers, encoder_norm)
    ...
    def forward(self, src, mask, query_embed, pos_embed, masks, src_all):
        ...
        if self.num_encoder_layers > 0:
            ...
            encoder_outputs = []
            for layer_output in memory_layers:
                encoder_outputs.append(layer_output)

    # Add a dense layer connecting each encoder to all subsequent encoders
            for i in range(len(encoder_outputs)):
                for j in range(i + 1, len(encoder_outputs)):
                    encoder_outputs[i] += self.dense_layer(encoder_outputs[j])
